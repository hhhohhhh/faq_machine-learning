...
------------      -------    --------    -----------
2022/1/14 9:51   wangfc      1.0         None
"""
import json
import time
from typing import Text, Optional, Dict, Any, Callable, Awaitable, List, Iterable
import uuid

from sanic import Sanic, Blueprint,response
from sanic.request import Request
from sanic.response import HTTPResponse
from apps.app_constants import DEFAULT_SENDER_ID
...
def register(
    ...
    # 增加 output_with_nlu_parse_data 参数，agent 控制输出的时候输出 nlu parse 信息
    """

    async def handler(message: UserMessage) -> None:
        # 使用 agent.async_handle_message 异步协程来处理数据逻辑
        await app.agent.handle_message(message,output_with_nlu_parse_data=output_with_nlu_parse_data)
