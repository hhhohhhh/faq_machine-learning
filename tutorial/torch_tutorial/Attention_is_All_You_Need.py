...
class Encoder(nn.Module):
    ...
    def forward(self, src, src_mask):
        ...
        pos = torch.arange(0, src_len).unsqueeze(0).repeat(batch_size, 1).to(self.device)  # pos = [batch size, src len]
        src = self.dropout(
            (self.tok_embedding(src) * self.scale) + self.pos_embedding(pos))  # src = [batch size, src len, hid dim]
        # encoder 的各层进行迭代
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask):
        ...
        # self attention
        _src, _ = self.self_attention(src, src, src, src_mask)  # _src = [batch size, src len, hid dim]

        # dropout, residual connection and layer norm
        src = self.layer_norm(src + self.dropout(_src))  # src = [batch size, src len, hid dim]

        # positionwise feedforward
        _src = self.positionwise_feedforward(src)  # _src = [batch size, src len, hid dim]
...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        Q = self.fc_q(query)  # query = [batch size, query len, hid dim]
        K = self.fc_k(key)  # key = [batch size, key len, hid dim]
        V = self.fc_v(value)  # value = [batch size, value len, hid dim]
        ...
        x = torch.matmul(self.dropout(attention), V)  # x = [batch size, n heads, seq len, head dim]
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, trg, enc_src, trg_mask, src_mask):
        "每个decorder层包括两部分：\
        1) 对于trg部分的 multihead self attention \
        2) 使用 trg作为query，src作为 Key和Value的 multihead self attention"
        # trg = [batch size, trg len, hid dim]
        # enc_src = [batch size, src len, hid dim]
        # trg_mask = [batch size, trg len]
        # src_mask = [batch size, src len]

        # decoder 部分进行 self attention
        _trg, _ = self.self_attention(trg, trg, trg, trg_mask)

        # dropout, residual connection and layer norm
        trg = self.layer_norm(trg + self.dropout(_trg))  # trg = [batch size, trg len, hid dim]

        # 进行 encoder attention
...
class Seq2Seq(nn.Module):
    ...
    def make_src_mask(self, src):
        # src = [batch size, src len] ---> src_mask = [batch size, 1, 1, src len]
        # so it can be correctly broadcast when applying the mask to energy = [batch size, n heads, seq len, seq len].
        src_mask = (src != self.src_pad_idx).unsqueeze(1).unsqueeze(2)
        return src_mask

    def make_trg_mask(self, trg):
        # trg = [batch size, trg len] ---> trg_pad_mask = [batch size, 1, trg len, 1]
...
def train(model, iterator, optimizer, criterion, clip):
    ...
    for i, batch in enumerate(iterator):
        ...
        # 输入 trg = [batch size, trg len-1]， 输出  output = [batch size, trg len - 1, output dim]
...
#                                             model=model, device=device, max_len=50, base_model=BASE_MODEL,batch_first=BATCH_FIRST)
#
