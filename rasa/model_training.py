...
async def _train_async_internal(
    ...
    # if stories.is_empty():
    # 当只使用 rules 作为 policy的时候，可以 stories is_empty
    if not stories:
        rasa.shared.utils.cli.print_warning(
...
async def train_nlu_async(
    ...
    logger.info(f"运行 {__name__}.train_nlu_async 函数")
...
async def _train_nlu_with_validated_data(
    ...
    logger.info(f"运行 {__name__}._train_nlu_with_validated_data 函数")
