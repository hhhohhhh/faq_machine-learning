...
async def train(
    ...
    **kwargs: Any,
    ...
    logger.info(f"开始训练 Trainer 对象 {'*'*100}")
    trainer = Trainer(
        nlu_config, component_builder, model_to_finetune=model_to_finetune
    )

    # 创建一个 远程的 persistor（远程保存模型的地址）
    persistor = create_persistor(storage)

    # 三种加载数据的方式
    logger.info(f"开始加载数据 {'*' * 100}")
    ...
    logger.info("统计分析和加载 training_data")
    ...
    logger.info(f"开始训练 NLU pipeline 成为一个 interpreter {'*' * 100}")
    interpreter = trainer.train(training_data, **kwargs)

    if path:
        logger.info(f"开始保存 trainer {'*' * 100}")
