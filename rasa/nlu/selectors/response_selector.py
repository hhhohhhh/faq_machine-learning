...
class ResponseSelector(DIETClassifier):
    ...
    def __init__(
        ...
        # build a specific response selector model for each retrieval intent：

        # 比如 retrieval_intent: ask_faq + chitchat 分别训练一个模型
        # TODO: 预测的时候如何首先预测得到 该 retrieval_intent


        self.retrieval_intent = None
        self.use_text_as_label = False # j
    ...
    def preprocess_train_data(self, training_data: TrainingData) -> RasaModelData:
        ...
        if self.retrieval_intent:
            # 训练的时候针对 retrieval_intent 下 不同 intent_response_key 或 response_text 进行训练
            # Each model would be trained on user utterance and response utterance pairs grouped under that retrieval intent.
        ...
        # INTENT_RESPONSE_KEY: 将所有 intent_response_key 作为 label 进行训练
        # RESPONSE： 将所有 response text 作为 label 进行训练
        ...
        # 根据不同的 label_attribute 属性，生成 _label_data
        self._label_data = self._create_label_data(
            training_data, label_id_index_mapping, attribute=label_attribute
        )
        # intent_examples : 和 nlu_examples 有何不同
    ...
    def process(self, message: Message, **kwargs: Any) -> None:
        ...
        # Get the exact intent_response_key and the associated responses for the top predicted label
        # label_intent_response_key= '开户咨询/科创板开通'
