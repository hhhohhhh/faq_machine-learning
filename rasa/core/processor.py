...
from rasa.shared.nlu.constants import INTENT_NAME_KEY, INTENT, ENTITIES, OUTPUT_INTENT
...
class MessageProcessor:
    ...
    async def handle_message(
        self, message: UserMessage,output_with_nlu_parse_data=False
    ) -> Optional[List[Dict[Text, Any]]]:
        """Handle a single message with this processor."""
        # preprocess message if necessary
        # rasa nlu interpreter 识别 message 并且更新 tracker
        ...
        if isinstance(message.output_channel, CollectingOutputChannel):
            if output_with_nlu_parse_data:
                # 增加 nlu的 解析结果
                messages = message.output_channel.messages
                latest_message = tracker.current_state().get('latest_message')
                # TODO: 使用 response_selector 获取 的 intent 和 confidence
                output_intent = latest_message.get(OUTPUT_INTENT)
                intent = latest_message.get(INTENT) #.get(INTENT_NAME_KEY)
                # intent_confidence = latest_message.get(INTENT).get('confidence')
                # intent.update({OUTPUT_INTENT:output_intent})
                ...
                if  messages.__len__()==0:
                    await  message.output_channel._persist_message(message =nlu_parse_data)
                    # messages.append(nlu_parse_data)
                else:
                    messages[0].update(nlu_parse_data)
    ...
    async def log_message(
        ...
        # nlu interpreter 识别 message 并且更新 tracker
    ...
    async def parse_message(
        ...
        if text.startswith(INTENT_MESSAGE_PREFIX):
            ...
            # 否则使用 interpreter 解析 message，得到 nlu 结果
