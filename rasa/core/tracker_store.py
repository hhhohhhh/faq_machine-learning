...
class TrackerStore:
    ...
    def serialise_tracker(tracker: DialogueStateTracker) -> Text:
        ...
        # 将 DialogueStateTracker 转化为 Dialogue 对象，再进行序列化
    ...
    def deserialise_tracker(
        ...
        # 初始化 DialogueStateTracker 对象
        tracker = self.init_tracker(sender_id)
        try:
            # parameters = Dict('name','events') 是 反序列化的 字典,
            # 其中 name是 conversation_id/sender_id, events 是 所有event的字典
            # 返回一个 Dialogue 对象，就是包含了多轮对话中的所有信息
...
class InMemoryTrackerStore(TrackerStore):
    ...
    self.store:  该属性在内存中生成一个 dict 字典来 保存了所有对话状态的，
                 key为 sender_id, value: serialised tracker
    """
    ...
    def save(self, tracker: DialogueStateTracker) -> None:
        ...
        # 将 serialised tracker 保存的 store属性当中，key= tracker.sender_id
