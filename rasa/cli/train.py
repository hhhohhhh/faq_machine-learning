...
def add_subparser(
    ...
    # 将 train 的 arguments 加入到  train_parser
    train_arguments.set_train_arguments(train_parser)

    # train 的子命令 还可以有子命令 train core 或者 train nlu
    ...
    # 每个子解析器就能知道应当执行哪个 Python 函数
    # logger.info(f"设置 train_nlu_parser 的 func = {run_nlu_training}")
...
def run_nlu_training(
    ...
    Returns:
        Path to a trained model or `None` if training was not successful.
    """
    logger.info(f"运行 {__name__}.run_nlu_training 函数")
