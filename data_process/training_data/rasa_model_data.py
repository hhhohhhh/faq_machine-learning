...
------------      -------    --------    -----------
2022/2/9 17:18   wangfc      1.0         None
"""
...
from utils.tensorflow.constants import SEQUENCE, BALANCED
...
class FeatureArray(np.ndarray):
    ...
    def __new__(
        ...
        # 使用 view casting 的方法创建 subclass
...
class FeatureSignature(NamedTuple):
    ...
    is_sparse: bool
    units: Optional[int]         # feature_dimension:    feature 的维数,如 128
    number_of_dimensions: int    # number_of_dimensions: feature 的维度，如 1，3或者4
...
class RasaModelData:
    ...
    'data' is a mapping of attribute name, e.g. TEXT, INTENT, etc.,
    and feature name,e.g. SENTENCE, SEQUENCE, etc.,
    to a list of feature arrays representing the actual features.
    'label_key' and 'label_sub_key' point to the labels inside 'data'.
    For example, if your intent labels are stored under INTENT -> IDS,
    'label_key' would be "INTENT" and 'label_sub_key' would be "IDS".
    """
    ...
    def __init__(
        ...
        escape_check_data_examples:bool=False
    ) -> None:
        """
        ...
        if escape_check_data_examples == False:
    ...
    def sort(self) -> None:
        ...
        data keys: ['text', 'label', 'sub_label']
        attribute_data keys: ['mask', 'sentence', 'sequence', 'sequence_lengths']
        """
    ...
    def number_of_examples(self, data: Optional[Data] = None) -> int:
        ...
        # 对于每个 data 中的每个key对应的  attribute_data，其中的每个feature 应该长度一致
        example_lengths = [
            len(f)
    ...
    def add_data(self, data: Data, key_prefix: Optional[Text] = None) -> None:
        ...
        """
        # key= ['text','intent'], sub_key = sequence or sentence, features= List[FeatureArray]
    ...
    def add_features(
        ...
        Args:
            ...
            update_num_examples : 新增参数控制是否检查 num_of_examples 属性
        """
        if features is None:
            return
...
class RasaBatchDataGenerator(RasaDataGenerator):
    ...
    RasaBatchDataGenerator:调用 Model.fit() 生成 DataHandler 对象
    DataHandler:调用 enumerate_epochs() 方法 对每个 epoch 进行迭代，在迭代中使用 catch_stop_iteration() 方法捕捉异常
    """
