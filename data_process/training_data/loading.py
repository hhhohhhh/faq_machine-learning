...
@file: 
@version: 
@desc:   rasa\shared\nlu\training_data\loading.py
@time: 2022/2/8 16:01 

@Modify Time      @Author    @Version    @Desciption
------------      -------    --------    -----------
2022/2/8 16:01   wangfc      1.0         None
"""
import os
from typing import Text, Optional
import json
from utils.io import list_files, read_file
from .foramats.rasa_yaml import RasaYAMLReader
...
def _load(filename: Text, language: Optional[Text] = "en") -> Optional["TrainingData"]:
    ...
    # json格式的时候：reader =  rasa.shared.nlu.training_data.formats.rasa.RasaReader
    # yaml格式的时候：reader =  rasa.shared.nlu.training_data.formats.rasa_yaml.RasaYAMLReader
    logger.info(f'运行 {__name__}._load,生成 reader={reader}，准备读取数据 from {filename}')
    if reader:
