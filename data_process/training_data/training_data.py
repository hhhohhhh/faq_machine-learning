...
@version: 
@desc:  
@time: 2022/1/19 15:25 

@Modify Time      @Author    @Version    @Desciption
------------      -------    --------    -----------
2022/1/19 15:25   wangfc      1.0         None
"""
import os
import copy
...
from utils.constants import INTENT, RESPONSE, ACTION_NAME, INTENT_RESPONSE_KEY, ENTITY_ATTRIBUTE_TYPE, \
    ENTITY_ATTRIBUTE_ROLE, NO_ENTITY_TAG, ENTITY_ATTRIBUTE_GROUP, TEXT, ENTITIES,DEFAULT_TRAINING_DATA_OUTPUT_PATH
from data_process.data_utils import check_duplicate_synonym, intent_response_key_to_template_key
from utils.common import lazy_property
from utils.finerprints import deep_container_fingerprint
from utils.io import read_file, raise_warning, write_text_file, is_likely_markdown_file, is_likely_yaml_file,\
    is_likely_json_file,markdown_file_extension
...
class TrainingData:
    ...
    def _load_lookup_table(lookup_table: Dict[Text, Any]) -> Dict[Text, Any]:
        ...
        if Path(potential_file).is_file():
            try:
                lookup_table["elements"] = read_file(
                    potential_file
                )
                return lookup_table
    ...
    def fingerprint(self) -> Text:
        ...
        return deep_container_fingerprint(relevant_attributes)
    ...
    def persist_nlu(self, filename: Text = DEFAULT_TRAINING_DATA_OUTPUT_PATH) -> None:
        if is_likely_json_file(filename):
            write_text_file(self.nlu_as_json(indent=2), filename)
        elif is_likely_markdown_file(filename):
            write_text_file(self.nlu_as_markdown(), filename)
        elif is_likely_yaml_file(filename):
            write_text_file(self.nlu_as_yaml(), filename)
        else:
            raise ValueError(
    ...
    def persist_nlg(self, filename: Text) -> None:
        if is_likely_yaml_file(filename):
            write_text_file(self.nlg_as_yaml(), filename)
        elif is_likely_markdown_file(filename):
            nlg_serialized_data = self.nlg_as_markdown()
            if nlg_serialized_data:
                write_text_file(nlg_serialized_data, filename)
        else:
            raise ValueError(
    ...
    def persist(
        ...
        return {"training_data": os.path.relpath(nlu_data_file, dir_name)}
