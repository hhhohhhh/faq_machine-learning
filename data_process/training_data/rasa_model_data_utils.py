...
@version: 
@desc:  
@time: 2022/2/11 16:47 

@Modify Time      @Author    @Version    @Desciption
------------      -------    --------    -----------
2022/2/11 16:47   wangfc      1.0         None
"""
...
#     from rasa.nlu.extractors.extractor import EntityTagSpec
      ...
      from models.extractors.extrator import EntityTagSpec


from data_process.training_data.message import Message
from data_process.training_data.rasa_model_data import FeatureArray, Data
from utils.constants import ENTITIES, TOKENS_NAMES, TEXT, ENTITY_ATTRIBUTE_TYPE, ENTITY_ATTRIBUTE_GROUP, \
    ENTITY_ATTRIBUTE_ROLE
from utils.tensorflow.constants import IDS, MASK
...
def featurize_training_examples(
    ...
    for example in training_examples:
        # 对于每个 example 生成 attribute_to_features 字典：每个属性对应多个 features 对象
...
def _surface_attributes(
    ...
    # 每个 attribute 对应的 features:  List[List[List["Features"]]],其中每个元素对应每个  example 的 features
...
def convert_to_data_format(
    ...
    # 每个 attribute 对应的 feature :  List[List[List["Features"]]],其中每个元素对应每个  example 的 features
...
def _feature_arrays_for_attribute(
    ...
    """
    # 获取对应的feature: List[List[List["Features"]]]
    features = (
        attribute_to_features[attribute]
    ...
    # _dense_features key={'sentence', 'sequence'},value = List[List[coo_matrix or np.ndarray]]
    ...
    for key, values in _sparse_features.items():
        if consider_dialogue_dimension:
            sparse_features[key] = FeatureArray(np.array(values), number_of_dimensions=4)
        else:
            # 将每个 featurizer 对应 feature 数据  List[ coo_matrix or np.ndarray]  转换 FeatureArray
            # v[0] 对应于 每个 featurizer ：
            # 维度0 表示 examples, 维度1 表示 featurizers对应的 List[coo_matrix or np.ndarray], 维度2 表示 coo_matrix or np.ndarray
    ...
    # attribute_to_feature_arrays key: {'sentence', 'sequence'},value = List[FeatureArray]
...
def _extract_features(
    ...
    for list_of_list_of_features in features:
        ...
        # 获取 sparse_features key = ['sequence', 'sentence'],value= List[coo_matrix]
        for key, value in dialogue_sparse_features.items():
            sparse_features[key].append(value)
        # 获取 dense_features key = ['sequence', 'sentence'],value= List[ np.ndarray]
    ...
    # dense_features:  key = ['sequence', 'sentence'], ,value= List[List[ np.ndarray]]
