...
@version: 
@desc:  
@time: 2022/1/19 14:11 

@Modify Time      @Author    @Version    @Desciption
------------      -------    --------    -----------
2022/1/19 14:11   wangfc      1.0         None
"""
...
from utils.constants import TEXT, INTENT, RESPONSE, DIAGNOSTIC_DATA, INTENT_RESPONSE_KEY, ENTITIES, METADATA, \
    FEATURE_TYPE_SENTENCE, FEATURE_TYPE_SEQUENCE, ENTITY_ATTRIBUTE_START, ENTITY_ATTRIBUTE_END,METADATA_INTENT, METADATA_EXAMPLE, RESPONSE_IDENTIFIER_DELIMITER, ACTION_NAME, \
    ACTION_TEXT
from utils.io import raise_warning
from utils.finerprints import deep_container_fingerprint
from .features import Features
from ..dataset.hsnlp_faq_knowledge_dataset import ATTRIBUTE
...
class Message:
    ...
    'time': None,
    'data': {
          # 读取 nlu.yml 中的 nlu 数据后的data：还可以包括'entities' + 'intent_metadata' + 'example_metadata'
          'text': '如何查询自助开户进度?',
          'intent': 'faq',
          'intent_response_key': 'faq/如何查询自助开户进度',
          # 进行分词后的增加的数据：
          'text_tokens':[rasa.nlu.tokenizers.tokenizer.Token,]
          'intent_tokens':
          'intent_response_key_tokens':
          }
      'features': [rasa.shared.nlu.training_data.features.Features]
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # 存储模型训练过程中的 Features： dense + sparse
    ...
    def build(
        ...
        **kwargs: Any,
        ...
        if intent: # 'advice/condition_list'
