...
class Transformer(object):
    ...
    def load_weights_from_checkpoint(self, checkpoint, mapping=None):
        ...
        logger.info("根据mapping从checkpoint加载权重:")
...
class BERT(Transformer):
    ...
    def apply_embeddings(self, inputs):
        ...
        """
        inputs = inputs[:]
        x = inputs.pop(0)      # x =(None, None)
        if self.segment_vocab_size > 0:
            s = inputs.pop(0)  # s =(None, None)
        if self.custom_position_ids:
            p = inputs.pop(0)
        else:
            p = None
    ...
    def apply_final_layers(self, inputs):
        ...
        if len(outputs) == 1:
            ...
            outputs = outputs[1:]
        # 当 with_pool  == False 的时候，返回 sequence 对应的向量
        return outputs
...
class ALBERT(BERT):
    ...
    def apply_main_layers(self, inputs, index):
        ...
        if attention_mask is not None:
            arguments['a_bias'] = True
            x.append(attention_mask)
        # MultiHeadAttention 是 mask-consumers layer,在 call的时候传入 mask 参数
        x = self.apply(
            inputs=x,
            layer=MultiHeadAttention,
            arguments=arguments,
...
def build_transformer_model(
    ...
    **kwargs):
    ...
    @author:wangfc
    @version:
    @description: 根据配置文件构建模型，可选加载checkpoint权重

    @params:
    增加参数 build_model + init_from_ckpt
    @return:
    """
    ...
    # 创建 transformer 的 layer
    transformer = MODEL(name=model,**configs)
    if build_model:
        # build model
        transformer.build(**configs)
    if init_from_ckpt and checkpoint_path is not None:
