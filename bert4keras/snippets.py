...
def sequence_padding(inputs, length=None, value=0, seq_dims=1, mode='post'):
    ...
    value： pad的值
    seq_dims: 序列的维度， input_ids 为 1 维
    ...
    """
    if length is None:
        # 获取 batch 中每个 sequence的 每个维度最大长度:  array([512])
    ...
    # 计算每个维度的 pad_width：pad_width[i] 对应维度的补偿个数， pad_width[i][0] 在该维度前面pad 个数， pad_width[i][1]在该维度后面pad 个数，
    ...
    for x in inputs:
        ...
        for i in range(seq_dims):
            if mode == 'post':
                # 前面 pad 0 个值，后面 pad length[i] - np.shape(x)[i]
                pad_width[i] = (0, length[i] - np.shape(x)[i])
            elif mode == 'pre':
                # 后面 pad 0 个值，后面 pad length[i] - np.shape(x)[i]
...
#     """数据生成器模版
#     """
#     def __init__(self, data,tokenizer,max_seq_length, data_type='train',batch_size=32, buffer_size=None):
...
#
#     def __repr__(self):
#         return f"{self.data_type} data_generator with length={self.data.__len__()}, batches={self.steps},batch_size={self.batch_size}"
#
#
#     def __len__(self):
#         return self.steps
