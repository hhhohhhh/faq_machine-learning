...
def test_new_axis():
    ...
    embedding_size =3
    all_labels_size =4
    sentence_vector_embed = np.random.randint(0,10,size=(batch_size,embedding_size))
    all_labels_embed = np.random.randint(0,10,size= (all_labels_size,embedding_size))
    a= sentence_vector_embed[:, tf.newaxis, :]
    b= all_labels_embed[tf.newaxis, :, :]
