...
------------      -------    --------    -----------
2022/1/19 9:49   wangfc      1.0         None
"""
import copy
import datetime
import os
...
# from rasa.constants import NLU_MODEL_NAME_PREFIX
from cli.cli_utils import print_error
from data_process.training_data.training_data import TrainingData
from models import components
from models.nlu.nlu_config import RasaNLUModelConfig, component_config_from_pipeline
from models.nlu.persistor import Persistor

from data_process.training_data.message import Message
from models.classifiers.classifier import IntentClassifier
from models.components import ComponentBuilder, Component
from models.extractors.extrator import EntityExtractor
from utils.common import module_path_from_object
from utils.constants import TEXT, INTENT, INTENT_NAME_KEY, PREDICTED_CONFIDENCE_KEY, ENTITIES, NLU_MODEL_NAME_PREFIX, \
    DEFAULT_MODELS_PATH

from utils.exceptions import MissingArgumentError, ModelNotFound

from utils.tensorflow.constants import EPOCHS
from utils.io import read_json_file, dump_obj_as_json_to_file, raise_warning, create_directory
import logging
...
class Metadata:
    ...
    def persist(self, model_dir: Text) -> None:
        ...
        dump_obj_as_json_to_file(filename, metadata, indent=4)
...
class Trainer:
    ...
    def _build_pipeline(
        ...
        for index, pipeline_component in enumerate(cfg.pipeline):
            ...
            logger.info("开始创建 NLU Pipeline的 第{}个component：{}".format(index, component_cfg))
            component = component_builder.create_component(component_cfg, cfg)
            # 验证  component 输入的参数是否在默认参数中
    ...
    def train(self, data: TrainingData, **kwargs: Any) -> "Interpreter":
        ...
        logger.info(f"运行 {__name__}.train 函数：开始训练 pipeline using the provided training data.")
    ...
    def persist(
        ...
        for i, component in enumerate(self.pipeline):
            ...
            # persist的时候返回信息，用于更新 从 file 加载模型的时候使用的 元数据 Metadata
            # DIETClassifier.persist() 返回的 {"file": file_name}
...
def load_interpreter_from_model(model_dir, use_compressed_model=True, run_eagerly=False) -> Interpreter:
    from cli.cli_utils import get_validated_path
    from models.model_utils import get_model, get_model_subdirectories

    model = get_validated_path(current=model_dir, parameter="model", default=DEFAULT_MODELS_PATH)
    try:
        model_path = get_model(model, use_compressed_model=use_compressed_model)
    ...
    interpreter = Interpreter.load(model_dir=nlu_model)
    return interpreter
