...
------------      -------    --------    -----------
2022/2/9 15:11   wangfc      1.0         None
"""
import math
import numpy as np
import logging
...
from models.featurizers.featurizer import DenseFeaturizer
from tokenizations.tokenizer import Tokenizer, Token
from utils.constants import TEXT, ACTION_TEXT, FEATURE_TYPE_SEQUENCE, FEATURIZER_CLASS_ALIAS, FEATURE_TYPE_SENTENCE, \
    LANGUAGE_MODEL_DOCS, TOKENS_NAMES, NUMBER_OF_SUB_TOKENS, NO_LENGTH_RESTRICTION, DENSE_FEATURIZABLE_ATTRIBUTES

# from rasa.nlu.utils.hugging_face.registry import model_class_dict
from utils.hugging_face.registry import model_class_dict, model_weights_defaults, MAX_SEQUENCE_LENGTHS, \
    model_tokenizer_dict, model_embeddings_post_processors, model_special_tokens_pre_processors, model_tokens_cleaners
from models import train_utils
from utils.tensorflow.constants import SEQUENCE_FEATURES, SENTENCE_FEATURES
...
class LanguageModelFeaturizer(DenseFeaturizer):
    ...
    defaults = {
        ...
        # 新增参数 from_pt，表示加载模型的来源 tf 或者 pytorch
        "from_pt": False,
        "cache_dir": None,
    ...
    @classmethod
    def create(
        cls, component_config: Dict[Text, Any], config =None#: RasaNLUModelConfig
    ) -> "DenseFeaturizer":
    ...
    def persist(self, file_name: Text, model_dir: Text) -> Optional[Dict[Text, Any]]:
        ...
        该方法在 rasa.nlu.model.Trainer.persisit() 中调用：
        主要为了使得训练的模型保存元数据，从而以后可以从file中加载

        参考 DIETClassifier.persist() 方法
        新建 self.persist() 方法，来保存从压缩文件中加载模型所需的文件：
            model_weights: config.json, pytorch_model, vocab.txt
        将它们保存在 nlu目录下
        Args:
    ...
    def _load_model_instance(self, skip_model_load: bool) -> None:
        ...
        self.model = model_class_dict[self.model_name].from_pretrained(
            self.model_weights,cache_dir=self.cache_dir,
            from_pt=self.from_pt # 增加参数 from_pt
        )
    ...
    def _lm_tokenize(self, text: Text) -> Tuple[List[int], List[Text]]:
        ...
        """
        # 对原始的 单个 token.text 转换为 token_id
        # 如果是 空格将会去除产生 []，如果是language vocabulary 之外的会产生 [UNK]
    ...
    def _post_process_sequence_embeddings(
        ...
        """
        # from rasa.nlu.utils.hugging_face.registry import model_embeddings_post_processors,
    ...
    def _tokenize_example(
        ...
        """
        # 获取经过 tokenizer 之后的 tokens
        tokens_in = message.get(TOKENS_NAMES[attribute])
        tokens_out = []
        token_ids_out = []

        for token in tokens_in:
            # TODO: 原始的 token.text 再进行 tokenize： 但是如果 tokens 格式和 处理后的 tokens 格式不同，实体识别的时候该如何处理
            ...
            if not split_token_ids:
                ...
                continue
            # 去除 split_token_strings 中 特殊符号： 如bert 中 “##”,但是保持 token_ids 用于预训练模型的预测
            ...
            # tokenizer 之后的一个token 可能经过 lm tokenizer 会转变为多个 sub_tokens
            token.set(NUMBER_OF_SUB_TOKENS, len(split_token_strings))
            # tokens_out 中的不包含 空格等字符的token_in
    ...
    def _get_model_features_for_batch(
        ...
        # shape = (batch_size, tokens_num, embedding_size)
        ...
        # sequence_nonpadded_embeddings 第一个维度表示每个 example,第2个维度表示其实际的 tokens,第3个维度 embedding_size
    ...
    def _get_docs_for_batch(
        ...
        """
        # 如果 HFTransformersNLP 已经生成 LANGUAGE_MODEL_DOCS 中的对应属性
        ...
        # 对原始 token 再使用 language model 进行 tokenize,去除 空格等字符
        ...
        for index in range(len(batch_examples)):
            doc = {
                SEQUENCE_FEATURES: batch_sequence_features[index],  # shape = (num_of_tokens, 312)
                SENTENCE_FEATURES: np.reshape(batch_sentence_features[index], (1, -1)), # 转换为 shape = (1,312)
            }
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        for attribute in DENSE_FEATURIZABLE_ATTRIBUTES:
            ...
            batch_start_index = 0
            batches = math.ceil(len(non_empty_examples)/batch_size)
            pbar = tqdm.tqdm(total=batches,desc=f"lm_featurizing attribute of {attribute}")
