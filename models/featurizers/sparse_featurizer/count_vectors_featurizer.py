...
from data_process.training_data.features import Features
...
from models.featurizers.featurizer import SparseFeaturizer
from models.model import Metadata
from models.nlu.nlu_config import NLUModelConfig
from tokenizations.tokenizer import Tokenizer
from utils.constants import TEXT, RESPONSE, ACTION_TEXT, MESSAGE_ATTRIBUTES, DENSE_FEATURIZABLE_ATTRIBUTES, \
    TOKENS_NAMES, INTENT, ACTION_NAME, INTENT_RESPONSE_KEY, MIN_ADDITIONAL_CVF_VOCABULARY, FEATURE_TYPE_SEQUENCE, \
    FEATURIZER_CLASS_ALIAS, FEATURE_TYPE_SENTENCE
from utils.io import raise_warning, json_pickle, json_unpickle, read_yaml_file
...
class CountVectorsFeaturizer(SparseFeaturizer):
    ...
    def _load_vocabulary_params(self) -> None:
        ...
        self.additional_vocabulary_size = self.component_config["additional_vocabulary_size"]

        # 新增 vocabulary_file 参数，读取指定的 vocabulary 作为 feature
        self.vocabulary_file = self.component_config.get("vocabulary_file",None)
        # self.attribute_to_vocabulary = self._load_vocabulary()

    def _load_vocabulary(self) -> Dict[Text,Set[Text]]:
        """
        @time:  2021/10/19 15:02
        @author:wangfc
        @version:
        @description: 读取 vocabulary_file ,作为 CounterVector 指定的 vocabulary
        ...
        if self.vocabulary_file and os.path.exists(self.vocabulary_file):
            vocabulary_data = read_yaml_file(self.vocabulary_file).get("vocabulary")
            # 默认的 vocabulary 是针对 text 属性的
            vocabulary = {TEXT :set(vocabulary_data)}
            return vocabulary
    ...
    def _get_additional_vocabulary_size(
        ...
        # from rasa.nlu.constants import MIN_ADDITIONAL_CVF_VOCABULARY
        return max(MIN_ADDITIONAL_CVF_VOCABULARY, int(existing_vocabulary_size * 0.5))

    def _add_custom_vocabulary(self,attribute: Text):
        """
        @time:  2021/10/19 16:44
        @author:wangfc
        @version:
        @description: 我们加入自定义的vocabulary,然后合并属性对应的 vocabulary

        @params:
        @return:
        """
        if self.attribute_to_vocabulary and self.attribute_to_vocabulary.get(attribute):
            custom_vocabulary  = self.attribute_to_vocabulary.get(attribute)
            original_vocabulary = self.vectorizers[attribute].vocabulary_
            current_vocabulary_size = len(original_vocabulary)
            # 比较原始的词典和自定义词典的差集
            added_custom_vocabulary =  set(custom_vocabulary ).difference(set(original_vocabulary.keys()))
            # 将自定义的词典加入的原有词典当中
            for word_index, word in enumerate(added_custom_vocabulary):
                index = current_vocabulary_size + word_index
                original_vocabulary[word] = index
            self._set_vocabulary(attribute, original_vocabulary)
            logger.info(f"属性{attribute}的对象{self.vectorizers[attribute].__class__.__name__},原有词典个数共{current_vocabulary_size},"
                        f"提供的自定义词典个数共{custom_vocabulary.__len__()},"
                        f"有{added_custom_vocabulary.__len__()}新词加入,当前共有字典个数共{self.vectorizers[attribute].vocabulary_.__len__()}")
    ...
    def _fit_vectorizer_from_scratch(
        ...
        # 对于需要增加 vocabulary 的属性增加 自定义的 vocabulary：
        self._add_custom_vocabulary(attribute)
    ...
    def train(
        ...
        cfg: Optional[NLUModelConfig] = None,
        **kwargs: Any,
    ) -> None:
        ...
        # 对于 所有的 attributes 进行训练 vectorizers
    ...
    def persist(self, file_name: Text, model_dir: Text) -> Optional[Dict[Text, Any]]:
        ...
        if self.vectorizers:
            ...
            if self._is_any_model_trained(attribute_vocabularies):
                ...
                json_pickle(featurizer_file, vocab)
    ...
    def _create_independent_vocab_vectorizers(
        ...
        for attribute in cls._attributes_for(parameters["analyzer"]):
            ...
            # 对于 TEXT 属性，我们可以取值 parameters["min_df"]，其他的属性使用默认值 1
            min_df = parameters["min_df"] if attribute == TEXT else cls.defaults.get("min_df")
            ...
            attribute_vectorizer = CountVectorizer(
                ...
                ngram_range=(parameters["min_ngram"], parameters["max_ngram"]),
                max_df=parameters["max_df"],
                min_df=min_df, # parameters["min_df"],
                max_features=parameters["max_features"],
                analyzer=parameters["analyzer"],
