...
@version: 
@desc:
使用rasa 的 nlu/components.py/ComponentMetaclass
@time: 2022/1/19 9:23 

@Modify Time      @Author    @Version    @Desciption
------------      -------    --------    -----------
2022/1/19 9:23   wangfc      1.0         None
"""
import itertools
import typing
from typing import Set, Iterable
from collections import defaultdict
from typing import Text, Optional, Dict, Any, List, Hashable, Type, Tuple

from data_process.training_data.message import Message
from models import train_utils
# from models import nlu_registry as registry
# from utils import registry
from utils.io import raise_warning

if typing.TYPE_CHECKING:
    # 如果不加 typing.TYPE_CHECKING，会造成循环import
...
from utils.constants import COMPONENT_INDEX, TRAINABLE_EXTRACTORS, CLASSIFIER
from utils.exceptions import MissingArgumentError, InvalidConfigException, MissingDependencyException
...
class ComponentMetaclass(type):
    ...
    metaclass -> class: 先定义metaclass，然后创建类。
    class -> instance:先定义类，然后创建实例。

    Classes as objects : classes are more than that in Python. Classes are objects too.
    ...
    Creating classes dynamically:
        type()函数可以查看一个类型或变量的类型，ComponentMetaclass 是一个class，它的类型就是type
        而 其实例，它的类型就是 class ComponentMetaclass
...
class Component(metaclass=ComponentMetaclass):
    ...
    The information is passed to other components by providing attributes to the so called pipeline context.
    The pipeline context contains all the information of the previous components
    a component can use to do its own processing.
    For example, a featurizer component can provide features that are used by another component down
    ...
    def __init__(self, component_config: Optional[Dict[Text, Any]] = None) -> None:
        ...
        # 使用 每个继承的类中的 defaults 参数设置默认的参数，并使用自定义的参数更新参数
        self.component_config = train_utils.override_defaults(
            self.defaults, component_config
        )

        # 不同的 component 通过以下两个属性来传递信息，
        # component.prepare_partial_processing() 的时候生成
        # component.partial_processing_context() 调用对 message 进行处理
        # partial_processing_pipeline ： previous components
        # partial_processing_context ： 之前的components 生成的信息，previous context
    ...
    @classmethod
    def create(
            cls, component_config: Dict[Text, Any], config: Dict[Text, Any]  # RasaNLUModelConfig
    ) -> "Component":
        """Creates this component (e.g. before a training is started).
        Method can access all configuration parameters.
        component_config   : 当个 component 的 config dict
        RasaNLUModelConfig : 整个 NLU pipeline 的 config 对象
        Args:
    ...
    def train(
            ...
            config: Optional[Dict[Text, Any]] = None,  # Optional[RasaNLUModelConfig] = None,
            **kwargs: Any,
    ) -> None:
        # from rasa.nlu.model.components import Component
    ...
    def prepare_partial_processing(
        ...
        """
        为 当下阶段的 component 准备好 previous pipeline 和 context:
    ...
    def partially_process(self, message: Any):  # Message) -> Message:
    ...
    def _add_component_name(
            self, pred_result: Dict[Text, Any], component_type=CLASSIFIER) -> Dict[Text, Any]:
        ...
        """
        pred_result[component_type] = self.name
        return pred_result
...
class ComponentBuilder:
    ...
    def __get_cached_component(
        ...
        from models.nlu_registry import get_component_class
        component_class = get_component_class(component_name)
        cache_key = component_class.cache_key(component_meta,
                                              model_metadata)  # 'HFTransformersNLP-bert-7e993dc979f3e1bd2b7f83b87fcf6f06'
        if (
    ...
    def create_component(
            self, component_config: Dict[Text, Any], cfg: NLUModelConfig
    ) -> Component:
        """Creates a component.
    ...
    def create_component_from_class(self, component_class: Type[C], **cfg: Any) -> C:
        ...
        return self.create_component(component_config, NLUModelConfig(cfg))
