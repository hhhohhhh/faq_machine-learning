...
------------      -------    --------    -----------
2022/2/9 17:25   wangfc      1.0         None
"""
...
from data_process.training_data.rasa_model_data import FeatureSignature
from . import layers
...
class ConcatenateSparseDenseFeatures(tf.keras.layers.Layer):
    ...
    def call(
        ...
        """
        # ex: counter_feature= SparseTensor [64, 28, 1100], lm_feature = Tensor or EagerTensor [64, 28, 312]
        features = inputs[0]
        dense_features = []
        for f in features:
...
class RasaFeatureCombiningLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        # Prepare sparse-dense combining layers for each present feature type  # {'sequence': True, 'sentence': True}
        self._feature_types_present = self._get_present_feature_types(
            attribute_signature)
        # 合并稀疏特征和稠密特征： _tf_layers['sparse_dense.sequence'] &  _tf_layers['sparse_dense.sentence']
        self._prepare_sparse_dense_concat_layers(attribute, attribute_signature, config)
        # 如果 sequence维度和 sentence维度不同，需要增加 ffnn 来统一维度：
...
class RasaSequenceLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        self._tf_layers: Dict[Text, Any] = {
            ...
            # 生成 sequence_embedding 和 sentence_embedding 上的 ffnn
    ...
    def call(
        ...
        Returns:
            ...
            attention_weights: Tensor containing self-attention weights received
                ...
                layers.
        """
        # sequence_features 中有不同的 tensor，表示不同的特征，
        # counter_feature= SparseTensor [64, 28, 1100], lm_feature = Tensor or EagerTensor [64, 28, 312]
        sequence_features = inputs[0]
        sentence_features = inputs[1]
        sequence_feature_lengths = inputs[2]
        # Combine all features (sparse/dense, sequence-/sentence-level) into one tensor,
        # also get a binary mask that has 1s at positions with real features and 0s at padded positions.
