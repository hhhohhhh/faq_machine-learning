...
@version: 
@desc:  
@time: 2021/6/21 17:33 

@Modify Time      @Author    @Version    @Desciption
------------      -------    --------    -----------
2021/6/21 17:33   wangfc      1.0         None
"""
...
def is_checkpoint_exist(init_checkpoint=None,checkpoint_data_suffix='data-00000-of-00001'):
    checkpoint_exist= False
    if init_checkpoint:
        pretrained_model_dir,checkpoint_name = os.path.split(init_checkpoint)
        checkpoint_data_filename = f"{checkpoint_name}.{checkpoint_data_suffix}"
        checkpoint_data_path  =os.path.join(pretrained_model_dir,checkpoint_data_filename)
        if os.path.exists(checkpoint_data_path):
            checkpoint_exist =  True
        else:
            logger.warning(f"checkpoint_data_path={checkpoint_data_path} 不存在")
    return checkpoint_exist
