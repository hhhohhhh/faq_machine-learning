...
------------      -------    --------    -----------
2021/3/1 16:50   wangfc      1.0         None
...
def set_batch_to_device(batch,device):
    ...
    if isinstance(batch, torch.Tensor):
        ...
        for key, value in batch.items():
            if isinstance(value, torch.Tensor):
                ...
                batch[key] = set_batch_to_device(value,device)
        return batch
        ...
        for value in batch:
            if isinstance(value, torch.Tensor):
                ...
                new_batch.append(set_batch_to_device(value,device))
            else:
...
#     logger.info(f"提取数据到 json.__len__()={tmp_json.__len__()} 从 {json_file_path}")
#     return tmp_json
#
#
...
#                   indent=indent,
#                   **kwargs)
#     logger.info(f"保存数据到obj.__len__()={obj.__len__()} 到 {json_file_path}")
#
#
...
def semantic_search(query_embeddings: Tensor,
    ...
    for query_start_idx in range(0, len(query_embeddings), query_chunk_size):
        ...
        for corpus_start_idx in range(0, len(corpus_embeddings), corpus_chunk_size):
            ...
            # cos_score_argpartition = np.argsort(-cos_scores,axis=1)[:, 0:top_k]
...
if __name__ == '__main__':
    triplet_data_path = os.path.join('data', 'triplet_dataset', 'triplet_data.json')
    os.chdir(os.path.dirname(os.path.dirname(__file__)))
    os.getcwd()
    json_data = default_load_json(triplet_data_path)
    import pandas as pd
    json_data = pd.read_json(triplet_data_path,orient='records',lines=True)
