...
if TYPE_CHECKING:
    from tokenizations.tokenizer import Token
...
def apply_bilou_schema(training_data: "TrainingData", exclude_entities:List[Text]=None) -> None:
    """Get a list of BILOU entity tags and set them on the given messages.

    Args:
        training_data: the training data
        exclude_entities ： 希望通过增加 exclude_entities，用于过滤 在 正则表达式 or lookup table 中出现的 entities
        暂时不做
    """
    logging.info("对训练数据进行 BILOU 实体标注")
    for message in training_data.nlu_examples:
        apply_bilou_schema_to_message(message,exclude_entities)


def apply_bilou_schema_to_message(message: "Message",exclude_entities:List[Text]=None) -> None:
    """Get a list of BILOU entity tags and set them on the given message.

    Args:
        message: the message
        exclude_entities :  对于部分 regex entity 进行排除标注，希望使其不出现在机器模型 entity extractor 当中
    """
    # from rasa.nlu.utils.bilou_utils import apply_bilou_schema_to_message
...
def _add_bilou_tags_to_entities(
    ...
    # start_pos 是 char 的 position
    for start_pos, end_pos, label in entities:
        # 使用 entity start_pos 对应的 token start_token_idx,
        start_token_idx = start_pos_to_token_idx.get(start_pos)
        # 获取 token end_token_idx， 从而可以使用 entity的位置信息找到 对应的 token indies
        end_token_idx = end_pos_to_token_idx.get(end_pos)
        # Only interested if the tokenization is correct:
        # 如果 entity 的 start_pos, end_pos 不在 start_pos_to_token_idx 或 end_pos_to_token_idx 之中，则返回为空
